...
get_online_bindata(struct fhistory *hptr, struct param *pptr)
{
  ...
  // Old version did highpass in place, which causes problems due to
  // windowing. New version copies into outbuf first.
  // Adam Janin 06/26/09

  for (i = 0; i < outbufptr->length; i++) {
    outbufptr->values[i] = inbufptr->values[i];
  }
    
  if (pptr->HPfilter == TRUE)
    {
      fvec_HPfilter(hptr, pptr, outbufptr);
    }

  for (i = 0; i < outbufptr->length; i++)
    {
      outbufptr->values[i] =
        window->values[i] * outbufptr->values[i];
    }
