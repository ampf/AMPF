...
struct fvec *audspec( const struct param *pptr, struct fvec *pspec)
{
  ...
  hz_in_fsamp = (float)(pspeclen - 1) 
    / (float)(pptr->sampfreq / 2.);

  /* chop the spectrum as indicated by early nyquist */
  pspeclen = 1+rint((pspeclen-1.0) * pptr->nyqhz/(pptr->sampfreq/2.));
  if (pspeclen > pspec->length) {
      pspeclen = pspec->length;		/* don't go beyond end */
  }
  ...
  if(cbweightptr == (struct fvec *)NULL)
    {
      ...
      {
	  /* use Mel */
	  get_mel_ranges(frange, cbrange, pptr, pspeclen, f_warp);
      }
      else if (pptr->trapezoidal == TRUE)
        {
